...
_is_npu = is_npu()
# Disable torch.compile on HIP: PyTorch inductor generates Triton kernels whose
# KernelMetadata lacks cluster_dims (a CUDA-only attribute), causing a crash on ROCm.
_is_hip = is_hip()


@torch.compile(dynamic=True, backend=get_compiler_backend(), disable=_is_npu or _is_hip)
